...
class AgentTaskModel(Base):
    ...
    acceptance_criteria: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="pending", server_default="pending")
    progress: Mapped[dict[str, object]] = mapped_column(JSONB, nullable=False, default=dict, server_default=text("'{\"done\":[],\"doing\":[],\"blocked\":[]}'::jsonb"))
    known_failures: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
    open_questions: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
    next_actions: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
    must_preserve: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
    environment_ref: Mapped[UUID | None] = mapped_column(PgUUID(as_uuid=True))
    artifact_refs: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
...
class TaskRequirementModel(Base):
    ...
    id: Mapped[UUID] = mapped_column(PgUUID(as_uuid=True), primary_key=True, default=uuid4)
    task_id: Mapped[UUID] = mapped_column(PgUUID(as_uuid=True), ForeignKey("agent_tasks.id", ondelete="CASCADE"), nullable=False)
    ...
    source_event_id: Mapped[UUID | None] = mapped_column(PgUUID(as_uuid=True), ForeignKey("session_events.id", ondelete="SET NULL"))
    evidence: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
...
class CheckpointModel(Base):
    ...
    id: Mapped[UUID] = mapped_column(PgUUID(as_uuid=True), primary_key=True, default=uuid4)
    task_id: Mapped[UUID] = mapped_column(PgUUID(as_uuid=True), ForeignKey("agent_tasks.id", ondelete="CASCADE"), nullable=False)
    parent_checkpoint_id: Mapped[UUID | None] = mapped_column(PgUUID(as_uuid=True), ForeignKey("checkpoints.id", ondelete="SET NULL"))
    ...
    state_hash: Mapped[str] = mapped_column(String(72), nullable=False)
    validator_report: Mapped[dict[str, object]] = mapped_column(JSONB, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
...
class ArtifactModel(Base):
    ...
    project_id: Mapped[str] = mapped_column(String(128), nullable=False)
    task_id: Mapped[UUID | None] = mapped_column(PgUUID(as_uuid=True), ForeignKey("agent_tasks.id", ondelete="SET NULL"))
    source_event_id: Mapped[UUID | None] = mapped_column(PgUUID(as_uuid=True), ForeignKey("session_events.id", ondelete="SET NULL"))
    metadata_json: Mapped[dict[str, object]] = mapped_column("metadata", JSONB, nullable=False, default=dict, server_default=text("'{}'::jsonb"))
...
class RetrievalTraceModel(Base):
    ...
    candidates: Mapped[list[object]] = mapped_column(JSONB, nullable=False)
    selected_memory_ids: Mapped[list[object]] = mapped_column(JSONB, nullable=False)
...
class ToolInvocationModel(Base):
    ...
    permissions: Mapped[list[object]] = mapped_column(JSONB, nullable=False, default=list, server_default=text("'[]'::jsonb"))
    ...
    artifact_id: Mapped[UUID | None] = mapped_column(PgUUID(as_uuid=True), ForeignKey("artifacts.id", ondelete="SET NULL"))
