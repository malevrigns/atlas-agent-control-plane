  ...
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto px-8 py-10 text-center max-sm:items-start max-sm:px-5 max-sm:py-6">
      <div className="max-w-xl">
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-3xl border border-blue-500/30 bg-blue-500/10 text-blue-400 shadow-2xl shadow-blue-950/30 max-sm:h-11 max-sm:w-11 max-sm:rounded-2xl">
          <Sparkles className="max-sm:h-5 max-sm:w-5" size={22} aria-hidden="true" />
        </div>
        <h2 className="mt-5 text-2xl font-semibold text-zinc-100 max-sm:mt-4 max-sm:text-lg">
          直接描述任务，Agent 会开始规划和执行
        </h2>
        <p className="mt-3 text-base leading-7 text-zinc-500 max-sm:text-sm max-sm:leading-6">
...
export function TimelineLoadingState() {
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-8 py-8 text-sm text-zinc-500 max-sm:px-5 max-sm:py-5">
      对话加载中...
    </div>
  );
}
