...
​        旧版教程把“生产构建、一键即起与 Nginx”与“测试、调试与可观测性”拆成了相邻两章。两者实际上属于同一条能力链：前者把基础结构立住，后者让它进入可用状态。本章将它们合并为前后两个阶段，保留原来的实现、验证与工程判断，同时减少能力尚未闭环时的章节跳转。

## 30.2 第一阶段：生产构建、一键即起与 Nginx

### 30.2.1 本阶段目标

​        学完本阶段后，你将能够：

​        换句话说，第一，理解为什么项目需要一键启动和停止脚本；第二，让 API 容器启动时自动执行数据库迁移；第三，使用 Docker Compose 健康检查控制服务启动顺序；第四，为 Nginx 补齐 SSE、WebSocket 和上传文件静态路径配置；第五，区分本地开发命令和 Compose 生产化启动命令；第六，使用轻量测试检查生产启动配置是否完整。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
​        从实现顺序看，第一，忘记执行数据库迁移；第二，启动顺序不稳定，Nginx 先起来但 API 还没健康；第三，SSE 流式响应被 Nginx 缓冲，前端看起来像“一次性返回”；第四，VNC WebSocket 需要升级请求头，普通 HTTP 代理不够；第五，Docker Compose 命令越来越长，不适合最终项目验收。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。

​        所以本阶段把“能跑”升级成“更像一个可交付项目的启动方式”。
...
​        放到工程语境里看，第一，不配置 HTTPS 证书；第二，不做生产域名；第三，不做容器日志采集；第四，不做蓝绿部署；第五，不做 CI/CD。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
​        展开来看，第一，根目录是否有启动和停止脚本；第二，API 容器是否使用启动脚本；第三，API 启动脚本是否会执行迁移；第四，Nginx 是否有流式响应、WebSocket 和上传静态路径配置。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
#### 30.2.6.6 更新 Docker Compose 健康检查和依赖
...
​        具体来说，第一，新增根目录一键启动脚本；第二，新增根目录停止脚本；第三，API 容器启动时自动执行数据库迁移；第四，Compose 增加健康检查依赖；第五，Nginx 增强 SSE、WebSocket 和上传静态路径配置；第六，新增生产启动配置测试；第七，README、Nginx README 和环境变量模板同步更新。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。

​        从这一阶段开始，项目不再依赖一串容易忘的手动命令，而是有了更稳定的一键启动入口。

## 30.3 第二阶段：测试、调试与可观测性

### 30.3.1 本阶段目标

​        学完本阶段后，你将能够：

​        换句话说，第一，理解 Agent 项目为什么需要测试、调试和可观测性一起建设；第二，为 API 请求增加 `X-Request-ID`，让一次请求可以被追踪；第三，新增系统诊断清单接口，集中展示常用排查命令；第四，在设置页展示 API、数据库、沙箱、工具、记忆和 Harness 的诊断入口；第五，使用 `unittest` 验证请求 ID 和诊断清单；第六，建立一套从页面问题追到后端日志、接口和容器状态的排查路径。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。

### 30.3.2 最终效果

​        本阶段结束后，任意 API 响应都会带上：
...
​        从实现顺序看，第一，Nginx 没有代理到 API；第二，API 容器没有启动；第三，API 能启动但数据库没连上；第四，数据库迁移没执行；第五，Redis 任务队列不可用；第六，Sandbox 没有启动；第七，VNC WebSocket 断开；第八，MCP 或 A2A 配置不可用；第九，某条 Agent 事件缺少必要 payload。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。

​        所以本阶段不只是“多写几个测试”，而是开始建立系统化排查能力。
...
​        放到工程语境里看，第一，不接 Prometheus；第二，不接 OpenTelemetry；第三，不接 Grafana；第四，不做集中日志平台；第五，不做真实链路追踪采样。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
​        展开来看，第一，浏览器或 curl 没有传 `X-Request-ID` 时，服务端自动生成；第二，调用方已经传了 `X-Request-ID` 时，服务端沿用它。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
#### 30.3.6.2 实现 RequestIdMiddleware
...
​        具体来说，第一，有没有覆盖核心子系统；第二，每条诊断项有没有命令；第三，每条诊断项有没有预期结果。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
​        换句话说，第一，`key`：稳定标识，用来给前端列表设置 key；第二，`name`：展示名称；第三，`category`：诊断分类，例如 core、agent、tooling；第四，`description`：说明这条诊断项排查什么；第五，`command`：可以复制执行的命令；第六，`expected`：正常情况下应该看到什么。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。

#### 30.3.6.5 实现 ObservabilityService
...
##### 30.3.6.5.1 为什么不在接口里直接探测所有服务

​        诊断接口如果每次都真的访问数据库、Sandbox、MCP、A2A 和浏览器，会有两个问题：

​        从实现顺序看，第一，它本身可能很慢；第二，某个外部服务卡住时，诊断接口也会卡住。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。

​        所以本阶段先做“诊断清单”，把排查入口结构化返回。

​        真正执行命令由用户或后续自动诊断流程完成。

#### 30.3.6.6 新增诊断 Schema 和路由
...
​        放到工程语境里看，第一，读取诊断项；第二，展示命令；第三，展示预期结果；第四，支持复制命令。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
​        展开来看，第一，请求 ID 串联日志；第二，诊断清单统一排查入口；第三，单元测试覆盖关键服务；第四，Harness 覆盖任务级回归；第五，Docker/Nginx 命令验证运行环境。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
​        具体来说，第一，点击左侧“设置”；第二，找到“系统诊断”面板；第三，确认能看到 API、数据库、Sandbox、Harness、MCP、A2A 等诊断项；第四，点击复制按钮，确认能复制命令。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。

### 30.3.9 阶段小结

​        本阶段完成了测试、调试与可观测性的第一层建设：

​        换句话说，第一，新增请求 ID 中间件；第二，所有 API 响应返回 `X-Request-ID`；第三，新增系统诊断领域实体和应用服务；第四，新增 `/api/observability/checks` 接口；第五，前端设置页新增系统诊断面板；第六，新增请求 ID 和诊断清单单元测试；第七，补充了从 API、数据库、Sandbox、工具、记忆、Harness 排查问题的入口。这些点放在一起看，构成了本阶段叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
