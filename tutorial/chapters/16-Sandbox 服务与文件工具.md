...
​        所以从本节开始，项目进入第 5 阶段：沙箱与内置工具。本节先搭建沙箱服务骨架，不急着实现文件和 Shell。先把服务边界、配置、响应、异常、Docker 和网关跑通，后面章节再在这个骨架上逐步加入真正的执行能力。
...
​        本节会先把最小可运行链路打通。`GET /api/status` 用来确认沙箱服务已经启动并能读取配置，`GET /api/supervisor/services` 用来提前固定进程状态接口的返回结构，`backend/sandbox/Dockerfile` 负责构建独立镜像，`docker-compose.yml` 负责把 `sandbox` 放进整体服务编排，`nginx/default.conf` 则负责把外部的 `/sandbox-api` 转发到沙箱容器内部的 `/api`。
​        本节也会刻意保留边界，不在这里实现文件 API、Shell API、真实 Supervisor、Chrome 或 VNC。这些能力都依赖一个稳定的沙箱服务入口，提前把它们全塞进来只会让第一版边界变得模糊。先让服务能独立启动、独立配置、独立被网关访问，后续章节再逐步把工具能力接上来。
...
nginx/README.md
nginx/default.conf
backend/sandbox/.dockerignore
backend/sandbox/Dockerfile
backend/sandbox/README.md
backend/sandbox/pyproject.toml
backend/sandbox/uv.lock
backend/sandbox/app/__init__.py
backend/sandbox/app/api/__init__.py
backend/sandbox/app/api/router.py
backend/sandbox/app/api/routes/__init__.py
backend/sandbox/app/api/routes/status.py
backend/sandbox/app/api/routes/supervisor.py
backend/sandbox/app/core/config.py
backend/sandbox/app/core/exceptions.py
backend/sandbox/app/core/handlers.py
backend/sandbox/app/core/logging.py
backend/sandbox/app/main.py
backend/sandbox/app/schemas/common.py
backend/sandbox/app/schemas/status.py
backend/sandbox/app/schemas/supervisor.py
backend/sandbox/app/services/__init__.py
backend/sandbox/app/services/supervisor_service.py
```

### 16.1.6 实施步骤
#### 16.1.6.1 创建 Sandbox 依赖配置
​        在 `backend/sandbox/` 目录创建 `pyproject.toml`：
...
​        `requires-python` 约束运行时版本，`dependencies` 只放本节真正需要的 FastAPI、配置读取和 Uvicorn。这里暂时不引入文件、Shell、浏览器相关依赖，是为了让骨架阶段保持轻量。`where = ["."]` 和 `include = ["app*"]` 则告诉 setuptools 只从当前 `sandbox` 目录收集 `app` 包，避免把仓库里其他服务误纳入沙箱包。

.1.6.1.2 为什么这样设计
​        沙箱服务和主 API 是两个独立应用。
​        它们以后会有不同依赖：主 API 关注数据库、Redis、LLM；沙箱会关注文件、Shell、浏览器和进程管理。
​        所以本节不复用 `backend/api/pyproject.toml`，而是给 `backend/sandbox/` 自己创建一套依赖配置。

#### 16.1.6.2 创建沙箱配置
​        创建 `backend/sandbox/app/__init__.py`：

```Python
"""AtlasAgent sandbox service."""
```

​        创建 `backend/sandbox/app/core/config.py`：
...
​        `sandbox_api_prefix` 表示沙箱服务内部的 API 前缀，本节保持为 `/api`。外部用户通过 Nginx 访问的是 `/sandbox-api`，容器内部服务看到的仍然是 `/api`，这可以让沙箱代码不用关心网关层路径。`workspace_dir` 是后续文件读写和 Shell 执行的根目录，先把它固定下来，之后所有执行能力才有边界可依。`supervisor_enabled` 和 `supervisor_services` 则为进程管理预留入口，本节先让它返回可解释状态，后面再接真实 Supervisor。
...
​        `workspace_dir` 是本节最重要的配置之一。虽然本节还没有文件 API，但后续所有文件读写都必须落在这个目录下，避免 Agent 随意访问容器里的任意路径。

#### 16.1.6.3 创建统一响应和异常处理
​        创建 `backend/sandbox/app/schemas/common.py`：
...
​        创建 `backend/sandbox/app/core/exceptions.py`：
...
​        创建 `backend/sandbox/app/core/logging.py`：
...
​        创建 `backend/sandbox/app/core/handlers.py`：
...
#### 16.1.6.4 创建状态接口
​        创建 `backend/sandbox/app/schemas/status.py`：
...
​        创建 `backend/sandbox/app/api/__init__.py`：

```Python
"""Sandbox API routes."""
```

​        创建 `backend/sandbox/app/api/routes/__init__.py`：

```Python
"""Sandbox route modules."""
```

​        创建 `backend/sandbox/app/api/routes/status.py`：
...
.1.6.4.1 字段含义
​        状态接口返回的是沙箱服务的最小身份信息。`service` 用来确认当前命中的确实是 Sandbox，而不是主 API 或前端页面；`environment` 和 `version` 用来定位运行环境和镜像版本；`status` 在本节固定为 `ok`，表示 FastAPI 应用已经启动并完成基础配置读取；`workspace_dir` 则提前暴露沙箱工作目录，方便后续验证文件 API 和 Shell API 是否都围绕同一个目录运行。
...
#### 16.1.6.5 创建 Supervisor 状态接口
​        创建 `backend/sandbox/app/schemas/supervisor.py`：
...
​        创建 `backend/sandbox/app/services/__init__.py`：

```Python
"""Sandbox application services."""
```

​        创建 `backend/sandbox/app/services/supervisor_service.py`：
...
​        创建 `backend/sandbox/app/api/routes/supervisor.py`：
...
​        本节常见状态是 `not_configured` 和 `unavailable`。前者表示配置上还没有启用真实 Supervisor，后者表示配置启用了，但当前镜像里没有安装 `supervisorctl`。等后续章节加入 Chrome、Xvfb、x11vnc 和 websockify 后，这个接口会开始返回更接近真实运行态的 `running`、`stopped` 等状态。

.1.6.5.2 代码讲解
​        本节没有真正安装 Supervisor，但先做接口。
...
#### 16.1.6.6 创建 API Router 和应用入口
​        创建 `backend/sandbox/app/api/router.py`：
...
​        创建 `backend/sandbox/app/main.py`：
...
​        这和主 API 的入口结构保持接近。这样后续读代码时，看到 `backend/api/app/main.py` 和 `backend/sandbox/app/main.py` 会有相同的理解方式。

#### 16.1.6.7 编写 Sandbox Dockerfile
​        创建 `backend/sandbox/.dockerignore`：
...
​        本节沙箱还没有 migrations、config、数据库等内容，所以镜像里只放入：
...
### 16.1.8 技术难点与亮点
​        本节的难点不在代码量，而在边界意识。Sandbox 是独立服务，就必须拥有自己的依赖配置、环境变量、启动入口、异常处理和健康检查。它不能一边声称独立，一边继续依赖主 API 的内部模块，否则后续工具执行出了问题，还是会牵扯到主 API 的运行环境。
​        另一个容易出错的地方是 Nginx 路径映射。外部路径是 `/sandbox-api`，沙箱服务内部路径是 `/api`，两者必须在网关层准确转换。如果这里写错，浏览器可能拿到 UI 的 404，也可能请求到主 API。Supervisor 接口同样体现了工程上的提前设计：即使本节还没安装 Supervisor，也要让调用方得到可解释状态，而不是让接口直接报 500。
...
​        `/api` 和 `/sandbox-api` 的区别也很容易被追问。`/api` 是服务内部前缀，主 API 和 Sandbox 都可以在各自服务内使用它；`/sandbox-api` 是网关对外暴露的路径，用来让浏览器和调试命令清楚区分访问目标。Docker Compose 里的 `expose` 只把端口暴露给内部网络，`ports` 才会映射到宿主机，这正好符合本节设计：外部统一走 Nginx，容器之间通过内部网络通信。
...
​        如果构建时提示 `ghcr.io/astral-sh/uv:0.11.15` 拉取失败，先用本地 `uv run uvicorn` 验证本节代码。等网络恢复后再构建 Docker 镜像。

### 16.1.11 小结
​        本节完成了 Sandbox 服务骨架。项目里新增了独立的 `sandbox` Python 应用，它有自己的依赖配置、配置读取、日志初始化、统一响应、统一异常、状态接口和 Supervisor 状态接口。随后，我们又为它准备了 Dockerfile，把它加入 Docker Compose，并通过 Nginx 的 `/sandbox-api` 对外暴露。
​        从这一阶段开始，AtlasAgent 不再只是主 API 里的一组业务服务，而是拥有了独立执行环境的基础。这个基础暂时还不执行命令，也不读写文件，但它已经回答了一个关键工程问题：Agent 后续要做的危险、阻塞、依赖外部进程的事情，应该发生在 Sandbox，而不是发生在主 API 里。本章后文会在这个基础上实现沙箱文件 API，并把主 API 的 FileTool 接到沙箱能力上。

### 16.1.12 代码
​        暂时无法在飞书文档外展示此内容

## 16.2 Sandbox 文件 API 与 FileTool 成形

### 16.2.1 本节目标
​        学完本节后，你将能够：
​        这一阶段要把前文搭好的 Sandbox 骨架变成真正能工作的执行环境。文件能力是 Agent 工具链里的第一块地基，因为后续无论是写代码、保存搜索结果、下载产物，还是把浏览器截图交给模型分析，最终都会落到文件系统里。
​        学完本节后，你应该能解释为什么文件工具必须运行在沙箱工作目录内，也能在 Sandbox 服务中实现列表、读取、写入、替换、删除、上传和下载接口。更重要的是，你要理解路径归一化为什么必须放在服务端完成，`../` 这类路径穿越为什么不能交给模型或前端自觉避免。最后，我们会在主 API 中封装 `SandboxFileClient`，把沙箱文件能力注册成 Agent 可调用的 FileTool，并通过 `/sandbox-api/files` 和 `/api/agent-core/tools` 验证完整链路。

### 16.2.2 最终效果
​        本节结束后，Sandbox 服务会新增一组文件接口：
...
### 16.2.3 本节要解决的问题
​        前文已经把 Sandbox 服务独立出来，并通过 `/sandbox-api/status` 确认沙箱服务可以访问。
...
​        所以本节先给 Sandbox 加文件 API，再让主 API 通过 FileTool 调用它。

### 16.2.4 本节技术方案
​        本节采用“Sandbox 提供文件 API，主 API 只通过客户端调用”的方案。
​        可选方案有三种：
...
​        本节会先在 Sandbox 中定义文件请求和响应模型，再实现 `SandboxFileService` 和 `/api/files` 路由，同时给读取、写入、上传加入大小限制。主 API 侧会增加 `SandboxFileClient`，并把文件能力注册成 `file_list`、`file_read`、`file_write`、`file_replace` 和 `file_delete`。Shell 命令执行、多沙箱实例管理、文件权限系统、二进制文件预览，以及 Planner/ReAct 对 FileTool 的真实自动调用策略，都留到后续章节逐步补上。
...
backend/sandbox/uv.lock
backend/sandbox/app/core/config.py
backend/sandbox/app/schemas/files.py
backend/sandbox/app/services/file_service.py
backend/sandbox/app/api/routes/files.py
backend/sandbox/app/api/router.py
backend/api/app/core/config.py
backend/api/app/infrastructure/sandbox/__init__.py
backend/api/app/infrastructure/sandbox/file_client.py
backend/api/app/infrastructure/agent_tools/sandbox_file.py
backend/api/app/infrastructure/agent_tools/builtin.py
```

### 16.2.6 实施步骤
#### 16.2.6.1 为 Sandbox 加入文件上传依赖
​        打开 `backend/sandbox/pyproject.toml`，在 `dependencies` 中加入：
...
#### 16.2.6.2 补充 Sandbox 文件配置
​        打开 `backend/sandbox/app/core/config.py`，把文件限制配置加到 `workspace_dir` 后面：
...
.2.6.2.1 字段含义
​        `workspace_dir` 是文件 API 的根边界。本地运行时，它默认指向 `backend/sandbox/workspace`，方便不用 Docker 也能调试；Docker Compose 运行时，它会被环境变量覆盖成容器里的 `/workspace`，也就是前文挂载出来的沙箱工作目录。
...
​        本节先用简单的字节数限制。后续如果要支持更复杂的文件策略，可以继续扩展成按任务、按用户、按沙箱实例的配额。

#### 16.2.6.3 定义文件 API 的请求和响应模型
​        创建 `backend/sandbox/app/schemas/files.py`：
...
​        `path` 使用相对路径，而不是绝对路径。调用方只知道 `notes/hello.txt`，不知道真实路径是 `backend/sandbox/workspace/notes/hello.txt` 还是容器里的 `/workspace/notes/hello.txt`。这样可以减少沙箱实现细节暴露。
...
#### 16.2.6.4 实现 Sandbox 文件服务
​        创建 `backend/sandbox/app/services/file_service.py`：
...
​        本节把安全校验放在 `SandboxFileService`，而不是放在每个路由函数里。这样未来 ShellTool、BrowserTool 如果也需要处理文件路径，可以复用同样的设计思路。

.2.6.4.6 小白最容易困惑的点
​        `Path(filename).name` 不是为了好看，而是为了去掉上传文件名里的路径。
​        如果浏览器或客户端上传的文件名是 `../../a.txt`，`Path(...).name` 会取出 `a.txt`。真正保存前仍然会经过 `_resolve_path()`，形成双重保护。

#### 16.2.6.5 创建 Sandbox 文件路由
​        创建 `backend/sandbox/app/api/routes/files.py`：
...
#### 16.2.6.6 注册 Sandbox 文件路由
​        打开 `backend/sandbox/app/api/router.py`，改成：
...
.2.6.6.1 代码讲解
​        前文只注册了 `status` 和 `supervisor`。
​        本节新增 `files.router` 后，Sandbox 应用才会真正暴露：
...
#### 16.2.6.7 更新 Docker Compose 环境变量
...
#### 16.2.6.8 在主 API 中增加 Sandbox 配置
​        打开 `backend/api/app/core/config.py`，在文件配置后加入：
...
#### 16.2.6.9 实现主 API 的 Sandbox 文件客户端
​        创建 `backend/api/app/infrastructure/sandbox/__init__.py`：

```Python
"""Sandbox API clients used by the main API service."""
```

​        创建 `backend/api/app/infrastructure/sandbox/file_client.py`：
...
#### 16.2.6.10 实现 Sandbox FileTool
​        创建 `backend/api/app/infrastructure/agent_tools/sandbox_file.py`：
...
​        所以本节把 Sandbox 返回的结构化数据格式化成文本。后续如果工具协议升级为结构化输出，可以把 `ToolCallResult.output` 扩展为 `dict` 或新增 `metadata` 字段。
...
#### 16.2.6.11 把 FileTool 注册进工具列表
​        打开 `backend/api/app/infrastructure/agent_tools/builtin.py`，新增 import：
...
### 16.2.8 技术难点与亮点
​        本节的技术难点首先是路径安全。文件工具不是把用户传进来的字符串直接交给 `open()`，而是必须把相对路径拼到 `workspace` 下，再用 `resolve()` 折叠真实路径，最后确认这个真实路径仍然位于工作目录内部。只要这一步漏掉，`../.env`、`/etc/passwd` 或 `notes/../../secret.txt` 都可能变成越界访问。
...
​        本节的亮点，是文件能力从一开始就运行在独立 Sandbox 服务中，而不是和主 API 耦合在一起。Sandbox API 面向程序，返回结构化 JSON；FileTool 面向 Agent，把结构化结果整理成可读文本；工具注册表则让这些能力可以被 `/api/agent-core/tools` 发现。`truncated` 字段也为后续上下文工程和分段读取留下了清楚的信号。
...
```Bash
cd /Users/atlas/Desktop/github/atlas-agents/backend/api
uv run python -m compileall app
```
...
### 16.2.11 小结
​        本节完成了 Sandbox 文件能力的第一版。Sandbox 侧新增了文件请求和响应模型，也新增了负责路径安全、读写限制、上传保存和下载路径处理的 `SandboxFileService`。在 HTTP 层，`/api/files` 已经覆盖列表、读取、写入、替换、删除、上传和下载这些基础动作。
