...
​        本章还要处理一个产品层面的问题。搜索结果不能只作为后端字符串存在，它需要在右侧工具预览面板里以更适合阅读的形式出现。普通工具输出可以直接放进 `pre` 文本框，但搜索结果更像一组网页条目，应该展示标题、链接和摘要。第 29 章刚整理出统一工具预览面板，本章正好让它承接第一种新工具类型。
...
​        `kind` 是整个设计里最关键的字段。第 29 章的工具预览面板已经可以解析浏览器截图，因为截图输出里有 `kind="browser_screenshot"`。本章延续同样的做法，搜索输出使用 `kind="search_results"`。前端不需要猜工具名称，也不需要解析自然语言，只要 JSON 能解析且 `kind` 匹配，就按搜索结果卡片展示。
...
### 30.6.10 前端解析搜索结果
​        第 29 章已经有 `ToolPreviewPanel`，本章在它里面增加搜索结果类型和解析函数。搜索结果的前端类型与后端 JSON 对齐。
...
## 30.11 常见问题
### 30.11.1 为什么不直接让模型自己搜索
​        模型本身没有稳定、可审计的外部搜索能力。即使某些平台提供联网能力，也很难在本项目的事件流和工具预览面板里复现它查了什么。把搜索做成工具以后，系统可以记录查询词、结果、供应商和调用时间，用户也可以看到来源链接。这是工程系统和黑盒联网回答之间的区别。

### 30.11.2 为什么不把 Bing 原始响应直接给前端
​        Bing 原始响应字段很多，而且属于供应商协议。前端如果直接依赖这些字段，后面换搜索供应商时就要改 UI。项目内只需要标题、链接和摘要，所以后端先把原始响应压成 `SearchResult`。前端看到的是项目自己的稳定结构，而不是 Bing 的临时细节。

### 30.11.3 为什么 SearchTool 返回字符串而不是对象
​        当前工具事件协议把工具输出保存为字符串，Shell 输出、文件输出和普通文本工具都是这样。SearchTool 如果直接返回对象，会让事件序列化和展示逻辑变得不统一。因此本章让工具返回 JSON 字符串，并用 `kind` 字段告诉前端如何解析。这种方式既保持协议一致，也保留了结构化展示能力。

### 30.11.4 没有 Bing API Key 时算不算搜索成功
​        不算真实搜索成功。没有 Key 时返回的是明确的配置提示，provider 也会显示为 `bing-disabled`。它的作用是验证工具链路和页面渲染，不是模拟真实网页检索。配置 Key 后，工具才会向 Bing Web Search API 发起真实请求并返回网页结果。

### 30.11.5 为什么关键词规则这么简单
​        这一章的重点是打通搜索工具链路，不是完成最终的智能工具选择器。关键词规则足够让教程里的搜索任务稳定触发 `search_web`，也容易调试。后续当模型能够输出结构化工具调用时，这段规则可以作为兜底逻辑保留，主路径会交给更强的工具选择策略。

### 30.11.6 搜索结果会不会直接进入最终回答
​        本章只负责让搜索结果进入工具事件和预览面板。最终回答如何引用这些结果，还需要后续的上下文注入、引用体系和回答生成逻辑配合。现在先把搜索结果变成可观察资料，后面才能讨论如何把资料安全地纳入最终回答。
...
## 30.13 下一章预告
​        搜索工具让 Agent 可以主动获取公开网页资料，但它仍然是项目内部预设的一种工具。下一章会进入 MCP 协议。MCP 的意义在于把外部工具服务标准化，让 Agent 不只使用自己代码里写死的工具，还能连接由其他进程、服务或团队提供的工具集合。理解了本章的工具协议和输出结构，下一章看 MCP 时会更容易把它放进同一套 Agent 工具体系里。
