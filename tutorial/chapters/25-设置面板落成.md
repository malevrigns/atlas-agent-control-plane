...
​        放到工程语境里看，第一，LLM 配置在 `/api/config/llm`；第二，MCP 状态在右侧工具预览环境页签；第三，A2A 状态也在右侧工具预览环境页签；第四，多 Agent 角色在第 24 章刚加入环境页签。这些点放在一起看，构成了本章叙述中需要连续理解的一条线索，而不是彼此孤立的项目清单。
...
## 25.6 实施步骤
### 25.6.1 定义设置接口 Schema

​        创建 `backend/api/app/schemas/app_settings.py`：
...
​        创建 `backend/api/app/application/app_settings_service.py`。
...
​        打开 `backend/api/app/presentation/http/routes/config.py`，新增设置接口。
...
​        创建 `frontend/web/app/lib/settings-api.ts`：
...
#### 25.6.4.1 为什么单独建`settings-api.ts`
...
​        创建 `frontend/web/app/components/settings-workspace.tsx`。
...
​        打开 `frontend/web/app/components/app-sidebar.tsx`，新增导航按钮：
...
​        打开 `frontend/web/app/page.tsx`，新增视图状态：
...
​        后续第 26 章进入产品化消息流时，设置页不会干扰对话工作台。
