...
### 36.6.6 让 ReAct 执行器选择多 Agent 工具
...
#### 36.6.6.1 为什么先判断多 Agent，再判断 A2A
...
- 问题：`/api/multi-agent/roles` 返回 404 怎么办？

​        解释：当前运行中的 API 容器可能还是旧镜像。执行 `docker compose build api`，然后执行 `docker compose up -d --force-recreate api nginx`。

- 问题：页面环境面板看不到“多 Agent 协作”怎么办？

​        解释：先确认 `curl http://localhost:8088/api/multi-agent/roles` 正常，再重新构建 UI：`docker compose build ui && docker compose up -d --force-recreate ui nginx`。

- 问题：工具事件没有命中 `multi_agent_collaborate` 怎么办？

​        解释：任务文本里要包含“多个 Agent”“分工”“协作”“评审”等关键词。后续真实模型工具选择接入后，会减少这种关键词依赖。

- 问题：为什么本章不直接启动多个 LLM？

​        解释：多 Agent 的事件协议、工具输出和前端展示需要先稳定。否则模型输出一变，页面和验证都会不稳定。后续 Agent Runner 章节会继续把确定性流程升级成真实多 Agent 执行。
...
​        第 37 章会进入设置面板，把 LLM、MCP、A2A 和多 Agent 配置收敛到统一设置入口，为后续更真实的多 Agent 运行和配置管理做准备。
