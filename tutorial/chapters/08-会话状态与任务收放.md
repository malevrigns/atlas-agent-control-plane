...
## 8.1 本章目标
​        第 7 章让事件可以流式推送，但会话本身还缺少任务控制感。用户看到事件在动，却不知道当前会话到底是空闲、运行中，还是已经被停止；事件来了之后，也没有一个清晰的位置告诉用户是否有未读内容。本章要补齐这层会话状态。
​        本章不新增数据库表，而是使用第 5 章已经预留的 `sessions.status` 和 `sessions.unread_count`。读者会编写停止会话接口、清除未读数接口，在 SSE 流里推送会话状态变化，并在前端增加会话控制栏。zustand store 也会继续承担状态协调职责，统一管理停止、清未读、运行中状态和会话列表更新。
...
## 8.3 本章要解决的问题
​        第 7 章已经完成 SSE 流式事件，但会话仍然缺少任务控制感。
​        真实的 Agent 会话通常需要知道：
...
## 8.6 实施步骤
### 8.6.1 扩展 SessionRepository 协议
​        打开 `backend/api/app/domain/sessions/repositories.py`，在 `SessionRepository` 中新增：
...
### 8.6.2 实现状态和未读数更新
​        打开 `backend/api/app/infrastructure/repositories/session_repository.py`，在 `SqlAlchemySessionRepository` 中新增：
...
### 8.6.3 扩展应用服务
​        打开 `backend/api/app/application/session_service.py`，在写入消息时增加未读数：
...
### 8.6.7 新增会话控制栏组件
​        创建 `frontend/web/app/components/session-control-bar.tsx`：
...
### 8.6.9 在 page.tsx 传入控制动作
​        打开 `frontend/web/app/page.tsx`，给 `ChatWorkspace` 传入新增 props：
...
#### 8.6.10.1 frontend/web/app/components/session-control-bar.tsx
...
#### 8.6.10.2 frontend/web/app/components/chat-workspace.tsx
...
​        项目亮点：
​        本章的项目亮点，是会话开始具备任务控制入口。前端控制栏被独立组件化，不污染聊天区域和页面入口；后端复用已有数据库字段，没有为了短期功能增加不必要迁移；第 9 章以后也可以继续接入后台任务和真实取消逻辑。
