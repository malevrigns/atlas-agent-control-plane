...
## 17.1 Sandbox Shell 与 ShellTool 成形
...
​        学完本节后，你应该能说明为什么 Shell 命令必须放在 Sandbox 中执行，也能理解 `asyncio.create_subprocess_shell` 如何启动子进程。更关键的是，本节不会把 Shell 做成“一次请求直接等完整结果”的接口，而是把命令抽象成 Shell 会话，支持启动、查询、等待、写入 stdin 和终止。主 API 侧会封装 `SandboxShellClient`，再把能力注册成 `shell_run` 和 `shell_status`。前端则不再新增孤立演示卡片，而是在既有工具结果区域里用终端样式展示 Shell 输出。

### 17.1.2 最终效果
​        本节结束后，Sandbox 服务会新增 Shell API：
...
​        所以本节把 Shell 能力放进 Sandbox 服务，再由主 API 通过 ShellTool 调用。

### 17.1.4 本节技术方案
​        本节采用“Shell 会话”的设计，而不是“一次请求直接返回全部结果”。
​        原因是命令可能有三种情况：
...
​        本节会在 Sandbox 中新增 Shell 请求和响应模型，实现 `SandboxShellService`，并把 `/api/shell` 路由挂进沙箱应用。主 API 侧会新增 `SandboxShellClient` 和 ShellTool，前端侧则改造已有工具结果区域，让 `shell_*` 工具结果以终端样式展示。
​        本节暂时不做多沙箱实例路由、命令白名单、执行审计、WebSocket 实时终端，也不会把 ShellTool 接入完整 ReAct 自动执行策略。这些能力都依赖更成熟的沙箱实例管理和工具预览体系。本章后文会处理 DockerSandbox 适配，第 19 章会集中整理工具预览面板。

### 17.1.5 本节删除和合并的临时 UI
​        本节不新增新的 Shell 演示卡片。
​        第 12 章留下的“Agent 记忆与工具协议”区域已经能选择工具、运行工具、展示工具结果。ShellTool 本质上也是工具，所以本节直接改造这个区域：
​        原来的工具选择和运行入口会保留下来，Memory 展示也会继续保留。真正变化的是工具结果渲染：当工具名以 `shell_` 开头时，结果区会使用深色终端样式，让用户一眼知道这里展示的是命令执行状态、退出码和标准输出。
​        同时，本节会删除第 12 章遗留的前端思维模型演示区域：
...
backend/sandbox/app/schemas/shell.py
backend/sandbox/app/services/shell_service.py
backend/sandbox/app/api/routes/shell.py
backend/sandbox/app/api/router.py
backend/api/app/core/config.py
backend/api/app/infrastructure/sandbox/shell_client.py
backend/api/app/infrastructure/agent_tools/sandbox_shell.py
...
### 17.1.7 实施步骤
#### 17.1.7.1 补充 Shell 配置
​        打开 `backend/sandbox/app/core/config.py`，在文件配置后加入：
...
​        所以本节先用 `shell_output_limit` 做内存保护。超出限制时只保留最后一段输出，并用 `output_truncated=true` 告诉调用方输出被裁剪过。

#### 17.1.7.2 定义 Shell 请求和响应模型
​        创建 `backend/sandbox/app/schemas/shell.py`：
...
#### 17.1.7.3 实现 Sandbox Shell 服务
​        创建 `backend/sandbox/app/services/shell_service.py`。
...
​        本节先把会话放在内存里，因为现在只有一个 Sandbox 服务实例。本章后文引入 DockerSandbox 后，会继续讨论多容器、多任务时会话归属的问题。

#### 17.1.7.4 创建 Sandbox Shell 路由
​        创建 `backend/sandbox/app/api/routes/shell.py`：
...
​        这也是本节和文件 API 最大的区别。文件 API 可以每次创建 service，因为文件状态在磁盘上；Shell API 的会话状态在内存里。

#### 17.1.7.5 注册 Shell 路由
...
#### 17.1.7.6 主 API 增加 Shell 客户端配置
​        打开 `backend/api/app/core/config.py`，在 Sandbox 配置后加入：
...
#### 17.1.7.7 实现主 API 的 Sandbox Shell 客户端
​        创建 `backend/api/app/infrastructure/sandbox/shell_client.py`：
...
#### 17.1.7.8 实现 ShellTool
​        创建 `backend/api/app/infrastructure/agent_tools/sandbox_shell.py`：
...
#### 17.1.7.10 改造前端工具结果展示
​        打开 `frontend/web/app/components/agent-core-panel.tsx`。
...
​        第二个重点是 Shell 的安全边界。
​        本节仍然只允许命令在 workspace 目录中执行。工作目录不能写绝对路径，也不能用 `../` 逃出 workspace。
​        第三个重点是前端收敛。
​        能力可以不断增加，但入口不应该无限增加。ShellTool 是工具能力，所以优先进入工具结果区域，而不是新增孤立演示面板。

### 17.1.9 技术难点与亮点
​        本节的技术难点在于 Shell 命令不是普通函数。进程启动后，stdout 和 stderr 需要后台持续收集，命令状态也要跨请求保存。否则启动命令的请求结束之后，下一次查询就找不到进程，也看不到后续输出。
...
​        本节的亮点，是 Shell 能力被隔离在 Sandbox 服务中，主 API 只通过 ShellClient 操作会话，不直接执行命令。ShellTool 复用第 12 章的工具协议，前端也没有继续堆新的演示卡片，而是开始把工具展示能力收敛到同一块区域。
...
#### 17.1.11.3 验证 Sandbox Shell API
​        启动命令：
...
### 17.1.12 小结
​        本节完成了 Sandbox Shell 能力。Sandbox 侧新增了 Shell 请求和响应模型，也新增了负责进程启动、会话记录、后台输出收集、等待、stdin 写入和终止的 `SandboxShellService`。路由层围绕会话 ID 暴露启动、查询、等待、写入和终止接口。
...
## 17.2 DockerSandbox 契合

### 17.2.1 本节目标
​        学完本节后，你将能够：
​        第 16 章和前文分别把文件与 Shell 能力接到了 Sandbox，但主 API 仍然是直接拿一个固定的 Sandbox 地址去调用。这个方式能跑通教学代码，却还没有形成真正的“沙箱管理”抽象。只要后续出现多任务、多容器或任务级隔离，散落在各处的 Sandbox URL 就会变成维护负担。
​        学完本节后，你应该能理解为什么主 API 需要一个 Sandbox 管理器，而不是让工具和路由到处直接拼 Sandbox 地址。本节会设计当前任务沙箱的创建、获取、健康等待和释放接口，在主 API 中封装 `DockerSandboxManager`，再通过它代理文件读取、文件写入和 Shell 命令执行。前端工作台右侧会出现“任务沙箱”状态面板，让用户在执行任务时能看到当前执行环境是否可用。

### 17.2.2 最终效果
​        本节结束后，主 API 会新增一组 Sandbox 管理接口：
...
### 17.2.3 本节要解决的问题
​        第 16 章和前文已经分别实现了 FileTool 和 ShellTool。
​        但现在主 API 的工具代码直接使用：
...
​        所以本节加入 `DockerSandboxManager`，先把“当前沙箱实例”的管理入口固定下来。

### 17.2.4 本节技术方案
​        本节不直接把 Docker socket 挂给主 API。
...
​        等本节之后需要一任务一容器时，可以把 `DockerSandboxManager` 的实现方式换成真实 Docker Engine 操作，路由和前端不用大改。
​        本节暂时不做这些内容：
​        本节不会把 `/var/run/docker.sock` 挂给主 API，也不会动态创建多个 Docker 容器。沙箱资源限制、任务和沙箱实例的一对一绑定、沙箱运行日志审计，都属于更靠后的生产化和安全加固内容。这里先把接口形状和适配层做稳定，避免过早把 Docker 权限和生命周期管理复杂度引入主 API。

### 17.2.5 本节删除和合并的临时 UI
​        本节不新增独立“DockerSandbox 演示页”。
​        沙箱状态是工作台的一部分，所以本节把它放进 `ChatWorkspace` 右侧：
...
​        同时，本节删除第 12 章留下的 Agent 核心演示前端入口。
...
backend/api/app/core/config.py
backend/api/app/schemas/sandbox.py
backend/api/app/infrastructure/sandbox/manager.py
backend/api/app/api/routes/sandboxes.py
backend/api/app/api/router.py
frontend/web/app/types.ts
frontend/web/app/lib/sandbox-api.ts
frontend/web/app/components/sandbox-status-panel.tsx
...
​        其中最后四个 `agent-core` 前端文件会在本节删除。后端的 Agent Core 接口和领域代码保留，因为后续 Agent 执行链路仍然会复用工具协议能力；删除的是页面上的临时演示 UI。

### 17.2.7 实施步骤
#### 17.2.7.1 补充主 API 沙箱配置
​        打开 `backend/api/app/core/config.py`，在 Sandbox 配置后加入：
...
.2.7.1.1 字段含义
​        `docker_sandbox_id` 是主 API 识别当前沙箱实例的稳定 ID。本节只有一个 Compose 沙箱，所以它可以固定为 `default`；以后如果变成一任务一沙箱，这个 ID 就可以替换成任务 ID、会话 ID 或容器 ID。
...
.2.7.1.2 代码讲解
​        本节只有一个 Compose 沙箱，所以 ID 固定为 `default`。
​        后续如果变成“一任务一沙箱”，这个 ID 可以替换成任务 ID、会话 ID 或容器 ID。

#### 17.2.7.2 定义主 API 沙箱响应模型
​        创建 `backend/api/app/schemas/sandbox.py`：
...
​        本节先让主 API 控制对外协议。后续 DockerSandbox 变成多实例后，前端仍然可以继续使用：
...
#### 17.2.7.3 实现 DockerSandboxManager
​        创建 `backend/api/app/infrastructure/sandbox/manager.py`：
...
​        本节先把“释放沙箱”这个接口形状做出来，后续真正一任务一容器时，再把实现方式替换成 Docker Engine 删除容器。

#### 17.2.7.4 新增 Sandbox 管理路由
​        创建 `backend/api/app/api/routes/sandboxes.py`：
...
#### 17.2.7.5 注册 Sandbox 路由
​        打开 `backend/api/app/api/router.py`，新增 `sandboxes`：
...
#### 17.2.7.6 新增前端 Sandbox API
​        创建 `frontend/web/app/lib/sandbox-api.ts`：
...
​        同时在 `frontend/web/app/types.ts` 中加入：
...
#### 17.2.7.7 新增任务沙箱状态面板
​        创建 `frontend/web/app/components/sandbox-status-panel.tsx`：
...
#### 17.2.7.8 把沙箱状态放进工作台
​        打开 `frontend/web/app/components/chat-workspace.tsx`，引入组件：
...
​        本节选择放在 `ChatWorkspace` 右侧，为后续 Shell、Browser、VNC 面板继续收敛做准备。

#### 17.2.7.9 删除旧演示面板
​        打开 `frontend/web/app/page.tsx`，删除第 12 章临时演示面板相关代码。
​        先删除这两个 import：
...
#### 17.2.7.10 页面加载当前沙箱状态
​        打开 `frontend/web/app/page.tsx`，引入 API：
...
### 17.2.9 技术难点与亮点
​        本节的技术难点在于把边界补完整。主 API 需要知道当前任务沙箱是否可用，但又不能把 Sandbox 的内部路径和容器细节直接暴露给前端。健康等待也不能简单地检查一次就失败，因为容器刚启动时可能还在加载服务，短时间内不可用并不代表整个环境不可用。
...
​        本节的亮点，是主 API 开始拥有统一的 Sandbox 管理入口。`/api/sandboxes/current` 把状态、健康等待、文件代理和 Shell 代理串在一起，同时为后续一任务一容器保留了稳定接口形状。前端也开始从“展示接口成果”转向“展示任务运行环境”。
...
#### 17.2.11.3 启动服务
​        如果本节修改了 API 代码，需要先重新构建 API 镜像：

```Bash
docker compose build api
```

​        如果本节修改了前端代码，需要重新构建 UI 镜像：
...
### 17.2.12 小结
​        本节完成了 DockerSandbox 适配的第一版。主 API 新增了 Sandbox 响应模型和 `DockerSandboxManager`，把当前沙箱的获取、确保、健康等待和释放统一到一个管理入口里。文件读取、文件写入和 Shell 运行也开始通过 `/api/sandboxes/current` 代理出去。
