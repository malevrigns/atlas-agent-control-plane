...
### 19.1.1 本节目标
​        学完本节后，你将能够：
​        第 18 章已经让浏览器截图进入事件记录，但截图只能告诉我们某一刻页面长什么样。真正做浏览器类 Agent 时，用户还会关心页面是否正在加载、点击之后有没有变化、输入框是否填上了内容。截图像一张证据照片，VNC 则更像一扇实时观察窗口。
​        学完本节后，你应该能理解为什么浏览器自动化还需要 VNC 远程桌面，也能讲清 `Xvfb`、`x11vnc`、`websockify` 和 `noVNC` 的分工。本节会在 Sandbox 容器中启动虚拟显示器、VNC 服务和 websockify，通过 Nginx 暴露 `/sandbox-vnc/`，再在前端工作台右侧嵌入浏览器远程桌面。到这里，浏览器能力就不再只是截图结果，而是具备可持续观察的界面。
...
### 19.1.3 本节要解决的问题
​        第 18 章已经能让 Sandbox 启动 Chromium 并截图。
​        第 18 章已经能让 Agent 调用 `browser_open` 和 `browser_screenshot`，并在事件记录里展示截图。
​        但是截图是离散结果，它只能回答：
...
​        所以本节加入 VNC 远程桌面，让用户可以在工作台中看到 Sandbox 里的浏览器实时画面。
...
​        本节选择让 Sandbox 只提供 websockify WebSocket，再让前端通过 `@novnc/novnc` SDK 渲染远程桌面：
...
​        本节暂时不做这些内容：
​        本节不做 VNC 密码、多会话多桌面、录屏、完整 noVNC 工具栏，也不做生产环境 HTTPS 和鉴权。这里仍然是课程里的本地开发闭环，重点是把画面从 Sandbox 容器送到前端工作台。安全和多租户能力会在后续生产化阶段再补。
...
backend/sandbox/scripts/start.sh
backend/sandbox/app/core/config.py
backend/sandbox/app/schemas/vnc.py
backend/sandbox/app/services/vnc_service.py
backend/sandbox/app/api/routes/vnc.py
backend/sandbox/app/api/router.py
frontend/web/app/types.ts
frontend/web/app/lib/sandbox-api.ts
frontend/web/app/components/vnc-panel.tsx
...
### 19.1.6 实施步骤
#### 19.1.6.1 补充 VNC 配置项
​        打开 `backend/sandbox/app/core/config.py`，在浏览器配置后加入：
...
​        `vnc_port` 是 x11vnc 在容器内监听的原生 VNC 端口，`vnc_web_port` 是 websockify 对外提供 WebSocket 的端口。`vnc_iframe_path` 是兼容字段，本节前端主要使用 `websocket_path` 连接 websockify。也就是说，配置层不是为了让前端硬编码端口，而是为了让 Sandbox 告诉前端应该连哪条观察通道。

.1.6.1.2 为什么这样设计
​        VNC 不只是前端页面，它依赖 Sandbox 容器里的多个进程。
​        这些配置让前端不需要硬编码端口和路径。后续如果多沙箱使用不同 noVNC 地址，只需要让状态接口返回不同的 `websocket_path`。

#### 19.1.6.2 新增 VNC 状态接口
​        创建 `backend/sandbox/app/schemas/vnc.py`：
...
​        创建 `backend/sandbox/app/services/vnc_service.py`：
...
​        创建 `backend/sandbox/app/api/routes/vnc.py`：
...
​        真正的浏览器打开网页仍然由第 18 章的 BrowserTool 完成。VNC 只是让用户观察这个浏览器。

#### 19.1.6.3 编写 Sandbox 启动脚本
​        创建 `backend/sandbox/scripts/start.sh`：
...
#### 19.1.6.4 更新 Sandbox Dockerfile
​        打开 `backend/sandbox/Dockerfile`，先把基础镜像固定到 bookworm：
...
.1.6.4.1 为什么 Dockerfile 要这样改
​        第 18 章只需要 headless Chromium，所以容器只要能运行 Playwright。
​        本节需要安装一组系统级桌面组件。这里把镜像固定为 `python:3.12-slim-bookworm`，是为了避免 `python:3.12-slim` 随 Debian 版本变化而带来不同的包解析结果。
...
​        `websockify` 通过 apt 安装成系统命令，不加入 `backend/sandbox/pyproject.toml`。这样 `uv sync` 不会因为 Python 包版 `websockify` 额外下载 `numpy`。noVNC 不放进容器，而是作为前端依赖运行在工作台中。

#### 19.1.6.5 更新 Compose 和 Nginx
...
.1.6.5.1 为什么 Nginx 要加 WebSocket 头
...
​        继续创建类型声明文件 `frontend/web/app/types/novnc.d.ts`：
...
​        这里补一个最小声明，只描述本节用到的构造函数、三个配置字段和 `disconnect()` 方法。

#### 19.1.6.7 前端新增 VNC 状态类型和 API
...
​        打开 `frontend/web/app/lib/sandbox-api.ts`，新增：
...
.1.6.7.1 为什么前端直接请求 `/sandbox-api/vnc/status`
...
#### 19.1.6.8 新增 VNC 面板组件
​        创建 `frontend/web/app/components/vnc-panel.tsx`。
​        这个文件在本节使用完整代码：
...
#### 19.1.6.9 把 VNC 面板接入工作台
​        打开 `frontend/web/app/components/chat-workspace.tsx`，导入：
...
​        打开 `frontend/web/app/page.tsx`，新增状态和加载逻辑：
...
### 19.1.8 技术难点与亮点
​        本节的技术难点在于显示链路。容器里没有真实显示器，所以必须用 Xvfb 创建虚拟显示器；VNC 是 TCP 协议，浏览器不能直接连接，所以还需要 websockify 把 VNC 转成 WebSocket；前端 noVNC SDK 的 WebSocket 连接又必须经过 Nginx 代理到 Sandbox 的 6080 端口。
​        Playwright 也要从 headless 模式切到有头模式，并且 `DISPLAY` 必须指向 Xvfb 创建的显示器。否则 Chromium 虽然能被启动，却不会出现在远程桌面画面里。本节的亮点，是浏览器能力从“截图结果”升级成“可观察的远程桌面”，而且 VNC 面板直接进入真实工作台，没有再新增孤立演示页。

### 19.1.9 面试考点
​        面试里问到这一阶段，可以把显示链路按顺序讲清楚。Xvfb 负责在无显示器容器里创建虚拟显示器，x11vnc 负责把这个显示器画面转成 VNC 服务，websockify 负责把 VNC TCP 连接转成浏览器可以使用的 WebSocket，`@novnc/novnc` 则在前端把 WebSocket 数据渲染成远程桌面。
...
​        这一阶段会安装 `xvfb`、`x11vnc`、`fluxbox` 和系统命令 `websockify`。构建时间会比普通 API 改动更长，但比 Debian `novnc` 包方案轻很多。
​        这里对 Sandbox 使用 `--no-cache --progress=plain`，是因为本节改了 Dockerfile 的系统依赖安装步骤。`--no-cache` 可以避免继续复用旧镜像层，`--progress=plain` 可以在网络失败时显示更完整的 apt 下载日志。
​        如果构建在 `apt-get install` 阶段报 `exit code: 100`，通常是系统包下载失败或镜像源网络波动。确认 Dockerfile 中没有安装 Debian 的 `novnc` 包后，可以稍后重试，或者按故障排查里的命令切换镜像源。
...
​        如果返回 `404 Not Found`，说明当前运行中的 Sandbox 还是旧镜像，容器里没有本节新增的 `app/api/routes/vnc.py`。重新执行：
...
### 19.1.11 小结
​        本节完成了 VNC 远程桌面的最小闭环。Sandbox 容器安装了 Xvfb、x11vnc、fluxbox 和系统命令 `websockify`，启动脚本会同时拉起虚拟显示器、VNC、websockify 和 FastAPI。Nginx 增加 `/sandbox-vnc/` 代理，前端使用 `@novnc/novnc` SDK 渲染远程桌面。
​        Sandbox 还新增了 `/api/vnc/status`，让前端可以读取 websockify 连接路径，而不是硬编码端口。从这一阶段开始，浏览器能力不只是“截图能看”，而是可以在工作台中持续观察。后续工具预览面板会把文件、Shell、Browser、截图和 VNC 继续收敛到更完整的产品体验中。

## 19.2 工具预览面板落成

### 19.2.1 本节目标
​        学完本节后，你应该能把前面几章分散出现的文件、Shell、浏览器截图和 VNC 观察能力，收敛到一个统一的右侧工具预览面板中。
​        第 16 章做了 FileTool，第 17 章做了 ShellTool，第 18 章让 BrowserTool 的截图进入事件记录，前文又把 VNC 远程桌面接进工作台。能力越来越多以后，如果每种工具都单独占一个卡片，右侧工作区会很快变成接口演示合集。真正的 Agent 产品不应该让用户在多个演示卡片之间来回找结果，而应该围绕“当前任务正在使用什么工具、产生了什么文件、执行环境现在是什么状态”来组织界面。
​        本节的目标，就是新增 `ToolPreviewPanel`，把工具调用、会话文件和执行环境放进一个分栏面板。工具页签展示最近的 `tool_called` 事件和截图预览；文件页签复用会话文件面板；环境页签集中展示当前任务沙箱和 VNC 远程桌面。整理完这一阶段后，右侧工作台会更像一个真正的执行观察区，而不是多个临时章节组件的堆叠。

### 19.2.2 最终效果
​        本节结束后，右侧工作台会从原来的多块面板：
...
​        因此，本节要做的不是新增一个能力，而是整理已有能力的展示方式。`ToolPreviewPanel` 会把最近工具调用、文件工作区、沙箱状态和 VNC 画面放到同一个面板中。这个改动看起来偏前端，但它代表项目从“章节演示”向“产品工作台”过渡。

### 19.2.4 本节技术方案
​        本节采用“统一面板 + 页签”的方案，而不是继续新增右侧卡片。
...
​        本节还会把 `EventTimeline` 中复杂的工具详情展示收回到 `ToolPreviewPanel`。事件时间线仍然保留，但它只显示事件流和工具名称摘要；详细参数、截图和最近工具调用列表交给工具预览面板处理。这样事件记录更像日志，工具预览更像观察面板，两个区域职责更加清晰。
...
​        本节主要改前端。后端的 FileTool、ShellTool、BrowserTool、Sandbox 管理接口和 VNC 状态接口都已经在前面章节完成，本节只是把它们在工作台里的展示关系重新组织起来。

### 19.2.6 实施步骤
#### 19.2.6.1 创建 ToolPreviewPanel 的输入模型
​        创建 `frontend/web/app/components/tool-preview-panel.tsx`，先引入图标、状态、子组件和类型：
...
​        注意这里没有根据工具名直接判断截图，而是解析输出内容。原因是第 18 章的截图工具把结果以 JSON 字符串放进 `ToolCallResult.output`，只要它符合 `kind: browser_screenshot` 的结构，前端就能展示图片。这样即使以后截图工具改名，只要输出协议不变，预览仍然可用。

#### 19.2.6.6 展示调用参数和截图图片
​        继续补充参数与截图组件：
...
​        截图预览使用 data URL，是本节最直接的展示方式。它不需要新增图片文件接口，也不需要先写入对象存储。后续如果截图越来越大，再把截图沉淀为文件对象，工具预览面板也可以继续复用同一个展示位置。

#### 19.2.6.7 工具摘要、图标和截图解析
​        继续实现摘要和辅助函数：
...
#### 19.2.6.8 接入 ChatWorkspace
​        打开 `frontend/web/app/components/chat-workspace.tsx`，把原来分散的面板替换成 `ToolPreviewPanel`。先导入组件：
...
#### 19.2.6.9 简化 EventTimeline 的工具展示
​        第 18 章曾经把工具参数、输出和截图都放进事件记录。现在这些详细内容已经进入 `ToolPreviewPanel`，事件记录就可以回到日志职责：显示事件发生了什么，以及大致是哪一个工具。
...
#### 19.2.6.10 页面状态继续由 Home 管理
​        `frontend/web/app/page.tsx` 仍然负责加载 API、数据库、Sandbox 和 VNC 状态，并把这些状态传给 `ChatWorkspace`。本节不把请求逻辑移进 `ToolPreviewPanel`，是为了避免展示组件直接耦合网络请求。
...
### 19.2.8 技术难点与亮点
​        本节的技术难点不是算法，而是前端信息架构。已有能力都能运行，但如何把它们放在同一个工作台里，并让用户快速理解它们的关系，是这一阶段要解决的核心问题。
​        `ToolPreviewPanel` 需要同时处理普通文本工具输出、浏览器截图、文件预览、沙箱状态和 VNC 画面。如果每一种输出都写成独立入口，页面会越来越散；如果全部塞进一个事件卡片，阅读又会很累。本节用页签分层解决这个问题。
​        这一阶段的亮点是，右侧工作台开始出现明确的信息分区：计划负责下一步，工具预览负责执行观察，上下文负责输入账单，事件记录负责历史轨迹。这个结构为后续 SearchTool、MCP、A2A 和多 Agent 协作继续扩展留出了稳定位置。

### 19.2.9 面试考点
​        面试里问到这一阶段，不要只说“我做了一个 tab 面板”。更好的回答是：随着工具类型增加，原来分散的面板不利于用户观察 Agent 执行过程，因此把工具调用、文件产物和执行环境统一收敛到 `ToolPreviewPanel`。
...
​        如果本节之前也修改过 Sandbox 或 API，可以一起启动：
...
### 19.2.11 小结
​        本节完成了工具预览面板的第一版。我们新增了 `ToolPreviewPanel`，用“工具、文件、环境”三个页签收敛工具调用、会话文件、任务沙箱和 VNC 远程桌面。`ChatWorkspace` 不再把这些能力散落摆放，而是把它们组织成一个更稳定的右侧观察区。
​        同时，事件记录被简化回日志职责，详细的工具参数、文本输出和浏览器截图由工具预览面板承载。从这一阶段开始，右侧工作台不再只是章节功能的堆叠，而是形成了更接近真实 Agent 产品的执行观察结构。

## 19.3 本章小结

​        完成“VNC 远程桌面临场”和“工具预览面板落成”两个阶段后，这条能力链已经形成闭环。读者仍然可以在每个阶段结束时单独运行验证，但理解上应把两者视作一个连续决策：先建立可靠边界，再让上层能力真正依赖它。
