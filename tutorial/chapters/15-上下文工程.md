...
## 15.1 本章目标
​        第 14 章让 Agent 执行进入了后台任务模型，但后台任务能跑起来并不代表它总能拿到合适的上下文。长任务执行到后半段时，系统可能已经积累了大量消息、事件和文件引用，如果把所有内容原样塞给模型，很快就会遇到上下文窗口、噪声干扰和文件内容过长的问题。
...
backend/api/app/domain/context_engineering/__init__.py
...
### 15.7.2 定义上下文工程领域模型
​        创建 `backend/api/app/domain/context_engineering/__init__.py`：

```Python
"""Context engineering domain objects."""
```

​        创建 `backend/api/app/domain/context_engineering/entities.py`：
...
### 15.7.3 编写 ContextEngineeringService
​        创建 `backend/api/app/application/context_engineering_service.py`：
...
### 15.7.7 新增前端 API 函数
​        打开 `frontend/web/app/lib/session-api.ts`，从类型中引入：
...
### 15.7.8 编写上下文面板组件
​        创建 `frontend/web/app/components/context-panel.tsx`：
...
​        打开 `frontend/web/app/page.tsx`，新增刷新函数：
