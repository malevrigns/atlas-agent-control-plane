...
> **最终实现修订：**本章先建立风险清单，再在后半完成强制 API Key、HttpOnly Web 会话、Sandbox 鉴权、VNC 网关认证、随机启动密钥，以及 MCP/A2A allowlist。最终源码包含这些控制，不只是报告风险。
...
## 48.10 最终实现修订：把风险清单变成强制边界

安全面板只能提示风险，不能代替访问控制。交付版在 Nginx、主 API、Sandbox 和执行配置四层同时收口：

1. Compose 默认只绑定 `127.0.0.1`，不会意外监听所有网卡；
2. 主 API 和 Sandbox 都校验 `X-Atlas-API-Key`，使用常量时间比较；
3. Web 首次输入 API Key 后，只把它交换成 `HttpOnly`、`SameSite=Strict` 会话 Cookie，浏览器脚本不保存原始密钥；
4. noVNC 通过 Nginx `auth_request` 复用同一认证边界，后端即使使用无密码 VNC 也不会被网关匿名公开；
5. `scripts/start.sh` 在首次启动时生成随机 API Key 和数据库密码，而不是使用仓库默认口令；
6. Redis 改为 `noeviction`，Nginx 增加认证限流与安全响应头。

直接调用 API 的示例：

```bash
curl -H "X-Atlas-API-Key: $ATLAS_API_KEY" \
  http://localhost:8088/api/sessions
```
...
API 返回会话 Cookie 后不再把密钥返回给页面。LLM 配置只保存 `api_key_env`；MCP stdio 命令、MCP HTTP 地址和 A2A 地址必须命中运维环境变量中的 allowlist，防止设置接口变成任意进程执行或任意出网入口。

这仍然是单租户共享密钥方案，不等于完整的生产身份系统。公网或多团队部署还应接入 OIDC、用户/服务身份、RBAC、TLS、密钥轮换和集中审计；共享密钥只解决“默认匿名公开”的阻断问题。
...
- 问题：有了 API Key，为什么还需要登录和权限？

​        解释：API Key 是单租户部署的最低访问门槛，不能区分用户，也不能表达角色和资源权限。公网或多租户环境仍需 OIDC、RBAC、TLS、轮换和逐用户审计。
