...
backend/api/app/domain/security/__init__.py
backend/api/app/domain/security/entities.py
backend/api/app/application/security_audit_service.py
backend/api/app/schemas/security.py
backend/api/app/presentation/http/routes/security.py
backend/api/app/presentation/http/router.py
backend/api/tests/test_security_audit_service.py
frontend/web/app/types.ts
frontend/web/app/lib/security-api.ts
frontend/web/app/components/security-panel.tsx
...
​        创建 `backend/api/tests/test_security_audit_service.py`：
...
​        创建 `backend/api/app/domain/security/__init__.py`：

```Bash
touch backend/api/app/domain/security/__init__.py
```

​        创建 `backend/api/app/domain/security/entities.py`：
...
#### 31.5.2.2 为什么放在 domain

​        `SecurityCheck` 不是数据库模型，也不是 HTTP 响应模型。

​        它描述的是项目内部对安全检查项的业务理解，所以放在 `domain/security/entities.py`。

### 31.5.3 实现 SecurityAuditService

​        创建 `backend/api/app/application/security_audit_service.py`：
...
### 31.5.4 新增 API Schema 和路由

​        创建 `backend/api/app/schemas/security.py`：
...
​        创建 `backend/api/app/presentation/http/routes/security.py`：
...
​        创建 `frontend/web/app/lib/security-api.ts`：
...
​        创建 `frontend/web/app/components/security-panel.tsx`。
